      ...
      .clauses = pvi(
          intoVampireIter(viras.quantifier_elimination(var, &*toElim))
            .map([premise, elimVar, toElim = Stack<Literal*>(*toElim), otherLits = std::move(otherLits)](auto litIter) {
              ...
              unsigned i = 0;
              for (auto lit : iterTraits(intoVampireIter(std::move(litIter)))) {
                images.push({toElim[i++], lit, RationalConstantType(1)});
                lits.push(lit);
              }
              for (unsigned j = 0; j < otherLits->size(); j++) {
                Literal* lit = (*otherLits)[j];
                images.push({lit, lit, RationalConstantType(1)});
                lits.push(lit);
              }
              auto cl = Clause::fromStack(lits,
